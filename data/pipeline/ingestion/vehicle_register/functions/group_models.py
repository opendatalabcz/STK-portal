...
def group_models(df: pd.DataFrame, model_primary_frequencies, **kwargs) -> pd.DataFrame:
    ...
    for make, mdf in by_make:
        ...
        twv = mdf[mdf['model_primary'].apply(lambda x: len(
            x.split(' '))) == 2]['model_primary'].to_numpy()
            ...
            if cnt < 25:
                ...
                if indices.size > 0 and sws_cnt[indices[0]] > 50:
                    ...
                    df.loc[(by_make.as_index) & (
                        df['model_primary'] == val), 'model_primary'] = fwp
                    continue

        # Obtain counts of two-word values.
        mwv = mdf[mdf['model_primary'].apply(lambda x: len(
            x.split(' '))) >= 3]['model_primary'].to_numpy()
        ...
        for val, cnt in zip(mwv_uniq, mwv_cnt):
            if cnt < 25:
                ...
                if indices.size > 0 and tws_cnt[indices[0]] > 50:
                    ...
                    df.loc[(by_make.as_index) & (
                        df['model_primary'] == val), 'model_primary'] = twp
                    continue
