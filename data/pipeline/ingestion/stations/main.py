...
def get_coords(row, session=None, token=None):
    data = {
...
def ingest(conn: Connection):
    ...
    stations = pd.read_excel(
        ...
        skiprows=[0, 1],
        usecols='A:H,K',
        names=['id', 'inspection_types', 'postal_code', 'city', 'street', 'company', 'phones', 'emails', 'nuts3'],
        dtype='str',
    )
    ...
    token = re.search(r'name="csrf_token"type="hidden"value="([^"]+)">', r.text.replace('\n', '').replace(' ', ''), re.MULTILINE).group(1)

    stations[['latitude', 'longitude']] = stations.apply(get_coords, axis=1, result_type='expand', session=session, token=token)

    # Add pure text fields for full-text-search.
    stations['inspection_types_fts'] = stations['inspection_types'].apply(join_array_type)
