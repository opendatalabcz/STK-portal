...
export default function InspectionResultByTopModel({
  linkToDetails = false,
}: {
  linkToDetails?: boolean;
}) {
  const { data: rawData } = useSWR(
    "/api/stations_inspection_success_by_model?order=year.desc,total.desc&limit=10",
    async (key) => {
  ...
  if (rawData == undefined) {
    return (
      <Card
        ...
        extra={
          linkToDetails && (
            <a href="/stations/inspection-result-by-model">Více</a>
          )
        }
      >
  ...
  return (
    <Card
      size="small"
      title={`Poměrný výsledek populárních modelů (${year})`}
      extra={
        linkToDetails && <a href="/stations/inspection-result-by-model">Více</a>
      }
    >
      <div className="h-64 md:h-96">
