...
export default function InspectionResultByMakeBrowser() {
  const [selectedYear, setSelectedYear] = useState<number>(latestYear);

  const { data: rawData } = useSWR(
    `/api/stations_inspection_success_by_make?year=eq.${selectedYear}&order=total.desc&limit=20`,
    async (key) => {
  ...
  if (rawData == undefined) {
    return (
      <Card title="Poměrný výsledek populárních značek">
