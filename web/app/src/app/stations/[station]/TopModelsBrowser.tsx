...
export default function TopModelsBrowser({ station }: { station: string }) {
  const [selectedYear, setSelectedYear] = useState<number>(latestYear);

  const { data: rawData } = useSWR(
    `/api/stations_top_models_by_station?station_id=eq.${station}&year=eq.${selectedYear}&order=count.desc&limit=10`,
    async (key) => {
      const res = await fetch(key);
      const data: TopModelsData[] = await res.json();
      return data;
    }
  );
  ...
  if (rawData == undefined) {
    return (
      <Card title="Nejčastěji kontrolované modely za rok" size="small">
  ...
  const data = {
    labels: rawData.map((item) => item.make + " " + item.model_primary),
    datasets: [
      {
  ...
  return (
    <Card
      title="Nejčastěji kontrolované modely za rok"
      size="small"
      extra={
